...
function SubmissionsContent() {
  ...
  async function loadSubmissions() {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/submissions?filter=${filter}`);
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-4 flex flex-col sm:flex-row gap-4">
        <div className="flex rounded-md shadow-sm">
          <button
            onClick={() => router.push('/admin/submissions?filter=all')}
          ...
          <button
            onClick={() => router.push('/admin/submissions?filter=pending')}
          ...
          <button
            onClick={() => router.push('/admin/submissions?filter=winners')}
                ...
                {filteredSubmissions.map((submission) => (
                  <tr key={submission.awardsId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right text-sm font-medium whitespace-nowrap">
                      <Link
                        href={`/admin/submissions/${submission.awardsId}`}
