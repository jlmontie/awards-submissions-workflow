...
/**
 * POST /api/surveys/admin/[id]/remind
 *
 * Update reminded_at for selected firms.
 * Body: { recipientIds: string[] }
 * Only applies to firms with status "sent" (not yet completed).
 */
export async function POST(
  ...
  try {
    ...
    const rRemindedAtCol = headers.indexOf('reminded_at');
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (!recipientIds.includes(row[rIdCol])) continue;
      if (row[rStatusCol] !== 'sent') continue; // Only remind firms that have been sent
      ...
      if (rRemindedAtCol !== -1) {
        await sheets.spreadsheets.values.update({
          spreadsheetId,
          range: `Survey Recipients!${columnLetter(rRemindedAtCol)}${sheetRow}`,
    ...
    return NextResponse.json(
      { error: 'Failed to mark as reminded' },
      { status: 500 },
    );
  }
}
